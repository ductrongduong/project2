...
#add notification to all observer
def add_notification(listing_id):
    listing = Listing.objects.get(pk=listing_id)
    users = listing.watched_by.all()
    for user in users:
        notification = Notification()
        notification.user = user
        notification.listing = listing
        notification.notification = f"{listing.title} is bid to {listing.current_price()}"
        notification.save()
